...
class G1_21(LeggedRobot):
    ...
    def step(self, actions):
        ...
        for _ in range(self.cfg.control.decimation):
            ...
            #     print(self.dof_names[i], (self.dof_pos - self.default_dof_pos_all)[0, i], self.dof_vel[0, i], self.torques[0, i])
            self.gym.set_dof_actuation_force_tensor(self.sim, gymtorch.unwrap_tensor(self.torques))
            self.gym.simulate(self.sim)
            self.gym.fetch_results(self.sim, True)
            self.gym.refresh_dof_state_tensor(self.sim)
        # for i in torch.topk(self.torques[self.lookat_id], 3).indices.tolist():
        #     print(self.dof_names[i], (self.dof_pos - self.default_dof_pos_all)[self.lookat_id, i], self.torques[self.lookat_id][i])
    ...
    def reset_idx(self, env_ids, init=False):
        ...
        if len(env_ids) == 0:
            return
        # RSI
        if self.cfg.motion.motion_curriculum and USE_MOTION:
